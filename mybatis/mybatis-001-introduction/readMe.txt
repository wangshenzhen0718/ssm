...
2．开发步骤
    ...
    *第四步:编写XxxMapper.xml文件
         在这个配置文件当中编写SQL语句。
         这个文件名也不是固定的，放的位置也不是固定，我们这里给它起个名字，叫做:CarMapper.xml把它放到类的根路径下。
    ...
    *第六步:编写MyBatis程序。(使用mybatis的类库，编写mybatis程序，连接数据库，做增删改查就行了。)
    在NyBatis当中，负责执行SQL语句的那个对象叫做什么呢?
    sqlSession
    sqlSession是专门用来执行SQL语句的，是一个Java程序和数据库之间的一次会话。
    要想获取SqlSession对象，需要先获取SqlSessionFactory对象，通过SqlSessionFactory工厂来生产SqlSession对象。怎么获取SqlSessionFactory对象呢?
...
4. mybatis中有两个主要的配置文件:
    其中一个是:mybatis-config.xml，这是核心配置文件，主要配置连接数据库的信息等。(一个)另一个是，XxxMapper.xml，这个文件是专门用来编写SQL语句的配置文件。
    t_user表，一般会对应一个UserMapper.xml insert delete update select
...
6．关于mybatis的事务管理机制。（深度剖析)
    ...
    *JDBC事务管理器:
    ...
    对于我们当前的单纯的只有mybatis的情况下，如果配置为:MANAGED那么事务这块是没人管的。
    没有人管理事务表示事务压根没有开启。
    *JDBC中的事务:
